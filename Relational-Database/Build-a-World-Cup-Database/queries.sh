...
echo $($PSQL "SELECT DISTINCT(name) FROM games FULL JOIN teams ON games.winner_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014
UNION SELECT DISTINCT(name) FROM games FULL JOIN teams ON games.opponent_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY name")
